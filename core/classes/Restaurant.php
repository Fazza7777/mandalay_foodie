...
class Restaurant{
    ...
    public function update($request){
        $id = $request["id"];
        $current =  $img = DB::table("restaurants")->where("id",$id)->first();
        $validation = [];
        if(empty($request["myanmar_name"])){
           $validation["m-shop"] = "Restaurant Name (Myanmar) must be require!";
        }
        ...
        //shop img
        if(empty($_FILES['shop']['tmp_name'])){     
            $res_imgName = $current->restaurant_image;         
        }else{
            $res_imgName = uniqid()."_".$_FILES['shop']['name'];
            $res_imgTmp = $_FILES['shop']['tmp_name'];
            move_uploaded_file($res_imgTmp,"upload/".$res_imgName);
        ...
        if(empty($_FILES['menu']['tmp_name'])){
            ...
            if(empty($current->menu)){
               $menu_image = "";     
                ...
                $menu_image = $current->menu;
            }
            
        }else{
            $menu_image = uniqid()."_".$_FILES['menu']['name'];
            $menu_imgTmp = $_FILES['menu']['tmp_name'];
            move_uploaded_file($menu_imgTmp,"upload/".$menu_image);
        }
       ...
       if(count($validation)>0){
        ...
        $meal_type = $request["meal_type"];
        $eatery_type = $request["eatery_type"];
        $fromHour = date("H", strtotime($request["from"]));
        $toHour = date("H", strtotime($request["to"]));
        //address
        $m_address = $request["m_address"];
        $e_address = $request["e_address"];
        $phone_one = $request["phone_one"];
    ...
    public static function singleRestaurant($id){
     ...
     public function searchForManage($request){
